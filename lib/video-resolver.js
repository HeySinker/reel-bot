...
async function resolveViaYtdlpUrl(pageUrl) {
  ...
  const { extractCaptionFromYtdlp } = require('./ytdlp-caption');
  const cookies = cookieArgsArray();

  const [stdout, caption] = await Promise.all([
    runYtdlp(['-g', '-f', 'best[ext=mp4]/best', '--no-playlist', ...cookies, pageUrl]),
    extractCaptionFromYtdlp(runYtdlp, cookieArgsArray, pageUrl),
  ]);
...
async function resolveViaCustomApi(pageUrl) {
  ...
  try {
    ...
    throw new Error(`محلّل الفيديو (Render): ${axiosErrorDetail(err)}`);
  }
}
...
async function resolveVideo(pageUrl) {
  ...
  if (IS_VERCEL && VIDEO_RESOLVER_URL) {
    throw new Error(`تعذّر استخراج الفيديو.\n\n${errors.join('\n')}`);
  }
